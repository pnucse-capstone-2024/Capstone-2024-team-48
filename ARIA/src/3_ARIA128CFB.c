...
void ARIA_CFB_encrypt(const char *req, const char *fax);
void compare_files(const char *fax, const char *rsp);

int main(int argc, char **argv) {
  const char *req_file = "ARIA128CFB.req";
  const char *fax_file = "ARIA128CFB.fax";
  const char *rsp_file = "ARIA128CFB.rsp";

  // 암호화를 수행하고 .fax 파일을 생성
  ARIA_CFB_encrypt(req_file, fax_file);
...
void ARIA_CFB_encrypt(const char *req, const char *fax) {
  Byte rk[16 * 17], c[16], mk[16], pt[16], iv[16], feedback[16];
  ...
  while (fgets(line, sizeof(line), req_fp)) {
    if (strncmp(line, "KEY = ", 6) == 0) {
      ...
      // CFB 모드
      Crypt(feedback, EncKeySetup(mk, rk, 128), rk, c);
      ...
      // 피드백 업데이트
      memcpy(feedback, c, 16);

      // ct_str 배열에 쓰기
      sprintf(ct_str, "CT = ");
      for (i = 0; i < 16; i++) {
          sprintf(ct_str + 5 + i * 2, "%02X", c[i]);
      }
