#include <stdio.h>
#include <stdlib.h>
#include <string.h>

typedef unsigned char Byte;

const Byte S[4][256];
const Byte KRK[3][16];

void DL(const Byte *i, Byte *o);
void RotXOR(const Byte *s, int n, Byte *t);
void printBlock(Byte *b);
int EncKeySetup(const Byte *w0, Byte *e, int keyBits);
int DecKeySetup(const Byte *w0, Byte *d, int keyBits);
void Crypt(const Byte *p, int R, const Byte *e, Byte *c);
void printBlockOfLength(Byte *b, int len);
void ARIA_CBC_encrypt(const char *req, const char *fax);
void compare_files(const char *fax, const char *rsp);

int main(int argc, char **argv) {
  const char *req_file = "ARIA128CBC.req";
  const char *fax_file = "ARIA128CBC.fax";
  const char *rsp_file = "ARIA128CBC.rsp";

  // 암호화를 수행하고 .fax 파일을 생성
  ARIA_CBC_encrypt(req_file, fax_file);

  // 생성된 .fax 파일과 .rsp 파일을 비교
  compare_files(fax_file, rsp_file);

  return 0;
}

// S-box와 KRK 상수 정의
const Byte S[4][256] = {
  // S-box type 1
  {
    
  },
  // S-box type 2
  {
    
  },
  // inverse of S-box type 1
  {
    
  },
  // inverse of S-box type 2
  {
    
  }
};

// KRK 상수 정의
const Byte KRK[3][16] = {
  {0x51, 0x7c, 0xc1, 0xb7, 0x27, 0x22, 0x0a, 0x94, 0xfe, 0x13, 0xab, 0xe8, 0xfa, 0x9a, 0x6e, 0xe0},
  {0x6d, 0xb1, 0x4a, 0xcc, 0x9e, 0x21, 0xc8, 0x20, 0xff, 0x28, 0xb1, 0xd5, 0xef, 0x5d, 0xe2, 0xb0},
  {0xdb, 0x92, 0x37, 0x1d, 0x21, 0x26, 0xe9, 0x70, 0x03, 0x24, 0x97, 0x75, 0x04, 0xe8, 0xc9, 0x0e}
};

// Diffusion Layer (확산 계층)
// 입력된 바이트 배열 i를 변환하여 출력 바이트 배열 o에 저장
void DL(const Byte *i, Byte *o) {
  Byte T;

  T = i[ 3] ^ i[ 4] ^ i[ 9] ^ i[14];
  o[ 0] = i[ 6] ^ i[ 8] ^ i[13] ^ T;
  o[ 5] = i[ 1] ^ i[10] ^ i[15] ^ T;
  o[11] = i[ 2] ^ i[ 7] ^ i[12] ^ T;
  o[14] = i[ 0] ^ i[ 5] ^ i[11] ^ T;
  T = i[ 2] ^ i[ 5] ^ i[ 8] ^ i[15];
  o[ 1] = i[ 7] ^ i[ 9] ^ i[12] ^ T;
  o[ 4] = i[ 0] ^ i[11] ^ i[14] ^ T;
  o[10] = i[ 3] ^ i[ 6] ^ i[13] ^ T;
  o[15] = i[ 1] ^ i[ 4] ^ i[10] ^ T;
  T = i[ 1] ^ i[ 6] ^ i[11] ^ i[12];
  o[ 2] = i[ 4] ^ i[10] ^ i[15] ^ T;
  o[ 7] = i[ 3] ^ i[ 8] ^ i[13] ^ T;
  o[ 9] = i[ 0] ^ i[ 5] ^ i[14] ^ T;
  o[12] = i[ 2] ^ i[ 7] ^ i[ 9] ^ T;
  T = i[ 0] ^ i[ 7] ^ i[10] ^ i[13];
  o[ 3] = i[ 5] ^ i[11] ^ i[14] ^ T;
  o[ 6] = i[ 2] ^ i[ 9] ^ i[12] ^ T;
  o[ 8] = i[ 1] ^ i[ 4] ^ i[15] ^ T;
  o[13] = i[ 3] ^ i[ 6] ^ i[ 8] ^ T;
}

// Right-rotate 128 bit source string s by n bits and XOR it to target string t
// 128비트 소스 문자열 s를 n 비트만큼 오른쪽으로 회전시키고 대상 문자열 t와 XOR 연산을 수행
void RotXOR(const Byte *s, int n, Byte *t) {
  int i, q;

  q = n/8; n %= 8;
  for (i = 0; i < 16; i++) {
    t[(q+i) % 16] ^= (s[i] >> n);
    if (n != 0) t[(q+i+1) % 16] ^= (s[i] << (8-n));
  }
}

// Encryption round key generation routine
// 암호화 라운드 키 생성 루틴
// w0 : 마스터 키, e : 암호화 라운드 키
int EncKeySetup(const Byte *w0, Byte *e, int keyBits) {
  int  i, R = (keyBits + 256) / 32, q;
  Byte t[16], w1[16], w2[16], w3[16];

  q = (keyBits - 128) / 64;
  for (i = 0; i < 16; i++) t[i] = S[i % 4][KRK[q][i] ^ w0[i]];
  DL(t, w1);
  if (R == 14)
    for (i = 0; i < 8; i++) w1[i] ^= w0[16+i];
  else if (R == 16)
    for (i = 0; i < 16; i++) w1[i] ^= w0[16+i];

  q = (q == 2) ? 0 : (q+1);
  for (i = 0; i < 16; i++) t[i] = S[(2 + i) % 4][KRK[q][i] ^ w1[i]];
  DL(t, w2);
  for (i = 0; i < 16; i++) w2[i] ^= w0[i];

  q = (q == 2) ? 0 : (q+1);
  for (i = 0; i < 16; i++) t[i] = S[i % 4][KRK[q][i] ^ w2[i]];
  DL(t, w3);
  for (i = 0; i < 16; i++) w3[i] ^= w1[i];

  for (i = 0; i < 16 * (R + 1); i++) e[i] = 0;

  RotXOR(w0, 0, e);
  RotXOR(w1, 19, e);
  RotXOR(w1, 0, e + 16);
  RotXOR(w2, 19, e + 16);
  RotXOR(w2, 0, e + 32);
  RotXOR(w3, 19, e + 32);
  RotXOR(w3, 0, e + 48);
  RotXOR(w0, 19, e + 48);
  RotXOR(w0, 0, e + 64);
  RotXOR(w1, 31, e + 64);
  RotXOR(w1, 0, e + 80);
  RotXOR(w2, 31, e + 80);
  RotXOR(w2, 0, e + 96);
  RotXOR(w3, 31, e + 96);
  RotXOR(w3, 0, e + 112);
  RotXOR(w0, 31, e + 112);
  RotXOR(w0, 0, e + 128);
  RotXOR(w1, 67, e + 128);
  RotXOR(w1, 0, e + 144);
  RotXOR(w2, 67, e + 144);
  RotXOR(w2, 0, e + 160);
  RotXOR(w3, 67, e + 160);
  RotXOR(w3, 0, e + 176);
  RotXOR(w0, 67, e + 176);
  RotXOR(w0, 0, e + 192);
  RotXOR(w1, 97, e + 192);
  if (R > 12) {
    RotXOR(w1, 0, e + 208);
    RotXOR(w2, 97, e + 208);
    RotXOR(w2, 0, e + 224);
    RotXOR(w3, 97, e + 224);
  }
  if (R > 14) {
    RotXOR(w3, 0, e + 240);
    RotXOR(w0, 97, e + 240);
    RotXOR(w0, 0, e + 256);
    RotXOR(w1, 109, e + 256);
  }
  return R;
}

// Decryption round key generation routine
// 복호화 라운드 키 생성 루틴
// w0 : 마스터 키, d : 복호화 라운드 키
int DecKeySetup(const Byte *w0, Byte *d, int keyBits) {
  int  i, j, R;
  Byte t[16];

  R = EncKeySetup(w0, d, keyBits);
  for (j = 0; j < 16; j++) {
    t[j] = d[j];
    d[j] = d[16 * R + j];
    d[16 * R + j] = t[j];
  }
  for (i = 1; i <= R / 2; i++) {
    DL(d + i * 16, t);
    DL(d + (R - i) * 16, d + i * 16);
    for (j = 0; j < 16; j++) d[(R - i) * 16 + j] = t[j];
  }
  return R;
}

// Encryption and decryption routine
// 암호화 및 복호화 루틴
// p: 평문, e: 라운드 키, c: 암호문
void Crypt(const Byte *p, int R, const Byte *e, Byte *c) {
  int i, j;
  Byte t[16];

  for (j = 0; j < 16; j++) c[j] = p[j];
  for (i = 0; i < R / 2; i++) {
    for (j = 0; j < 16; j++) t[j] = S[j % 4][e[j] ^ c[j]];
    DL(t, c);
    e += 16;
    for (j = 0; j < 16; j++) t[j] = S[(2 + j) % 4][e[j] ^ c[j]];
    DL(t, c);
    e += 16;
  }
  DL(c, t);
  for (j = 0; j < 16; j++) c[j] = e[j] ^ t[j];
}

// 블록 데이터를 지정된 길이만큼 출력
void printBlockOfLength(Byte *b, int len) {
  int i;

  for (i = 0; i < len; i++, b++) {
    printf("%02x", *b);
    if (i % 4 == 3 && i < len - 1) printf(" ");
  }
}

// 16바이트 블록 데이터를 출력
void printBlock(Byte *b) {
  printBlockOfLength(b, 16);
}

// 파일 입출력으로 주어진 입력 파일에서 평문과 키를 읽어들이고, 암호화를 수행한 후 암호문을 출력 파일에 기록
void ARIA_CBC_encrypt(const char *req, const char *fax) {
  Byte rk[16 * 17], c[16], mk[16], pt[16], iv[16], xor_block[16];
  char line[128], ct_str[64];
  int i, first_block = 1;

  FILE *req_fp = fopen(req, "r");
  FILE *fax_fp = fopen(fax, "w");

  while (fgets(line, sizeof(line), req_fp)) {
    if (strncmp(line, "KEY = ", 6) == 0) {
    // 키
    for (i = 0; i < 16; i++) {
      sscanf(&line[6 + 2 * i], "%2hhx", &mk[i]);
    }
    } else if (strncmp(line, "IV = ", 5) == 0) {
    // 초기 벡터(IV)
    for (i = 0; i < 16; i++) {
      sscanf(&line[5 + 2 * i], "%2hhx", &iv[i]);
    }
    // IV값으로 xor블럭을 초기화
    memcpy(xor_block, iv, 16);
    } else if (strncmp(line, "PT = ", 5) == 0) {
    // 평문
    for (i = 0; i < 16; i++) {
      sscanf(&line[5 + 2 * i], "%2hhx", &pt[i]);
    }

    // XOR with previous ciphertext (or IV for the first block)
    for (i = 0; i < 16; i++) {
      pt[i] ^= xor_block[i];
    }

    // Encrypt plaintext
    Crypt(pt, EncKeySetup(mk, rk, 128), rk, c);

    // Update xor_block with the current ciphertext for the next block
    memcpy(xor_block, c, 16);

    // ct_str 배열에 쓰기
    sprintf(ct_str, "CT = ");
    for (i = 0; i < 16; i++) {
      sprintf(ct_str + 5 + i * 2, "%02X", c[i]);  // Using %02X for uppercase hex
    }

    // fprintf 함수로 .fax 파일에 저장
    fprintf(fax_fp, "%s\n", ct_str);
    }
  }

  fclose(req_fp);
  fclose(fax_fp);
}

// 파일 입출력으로 암호문 파일과 응답 파일을 비교하여 일치 여부를 출력
void compare_files(const char *fax, const char *rsp) {
  char fax_line[128], rsp_line[128];
  int total = 0, matches = 0;

  FILE *fax_fp = fopen(fax, "r");
  FILE *rsp_fp = fopen(rsp, "r");

  while (fgets(fax_line, sizeof(fax_line), fax_fp) && fgets(rsp_line, sizeof(rsp_line), rsp_fp)) {
    if (strcmp(fax_line, rsp_line) == 0) {
      matches++;
    }
    total++;
  }

  fclose(fax_fp);
  fclose(rsp_fp);

  printf("Matches: %d/%d\n", matches, total);
  matches == total ? printf("PASS") : printf("FAIL"); // 암호알고리즘 검증기준 요구사항
}
