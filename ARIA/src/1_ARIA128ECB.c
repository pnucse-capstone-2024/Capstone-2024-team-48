...
void ARIA_ECB_encrypt(const char *req, const char *fax);
void compare_files(const char *fax, const char *rsp);

int main(int argc, char **argv) {
  const char *req_file = "ARIA128ECB.req";
  const char *fax_file = "ARIA128ECB.fax";
  const char *rsp_file = "ARIA128ECB.rsp";

  // 암호화를 수행하고 .fax 파일을 생성
  ARIA_ECB_encrypt(req_file, fax_file);
...
void ARIA_ECB_encrypt(const char *req, const char *fax) {
  Byte rk[16 * 17], c[16], mk[16], pt[16];
  ...
  while (fgets(line, sizeof(line), req_fp)) {
    if (strncmp(line, "KEY = ", 6) == 0) {
      ...
      sprintf(ct_str, "CT = ");
      for (i = 0; i < 16; i++) {
        sprintf(ct_str + 5 + i * 2, "%02X", c[i]);  // `CT = `이후에 쓰기 위해 `+5`, 16진수 대문자로 저장하기 위해 02X 사용
      }
